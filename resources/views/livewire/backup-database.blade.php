<div class="space-y-6">
    ...
    @if($pesan)
        <div @class([
            ...
            'bg-emerald-50 border-emerald-200 text-emerald-800' => $tipePesan === 'sukses',
            'bg-red-50 border-red-200 text-red-800'             => $tipePesan === 'error',
        ])>
            @if($tipePesan === 'sukses')
            ...
            @endif
            <div class="flex-1 font-medium">{{ $pesan }}</div>
            <button wire:click="$set('pesan', null)" class="text-gray-400 hover:text-gray-600 flex-shrink-0">
    ...
    <div class="overflow-hidden rounded-xl bg-white border border-gray-200 shadow-sm p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
                <h3 class="text-base font-semibold text-gray-900">Backup Database Server Monitor</h3>
                <p class="text-xs text-gray-500 mt-1">
                    Unduh file cadangan database <code class="bg-gray-100 px-1.5 py-0.5 rounded font-mono text-gray-700">.sql</code> berisi seluruh data sekolah terhubung, rilis, pengumuman, dan riwayat heartbeat.
                </p>
    ...
    <div class="overflow-hidden rounded-xl bg-white border border-gray-200 shadow-sm p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
                <h3 class="text-base font-semibold text-gray-900">Restore Database dari Server Lain</h3>
                <p class="text-xs text-gray-500 mt-1">
                ...
                <form wire:submit="restoreSql" class="mt-4 space-y-3">
                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center bg-gray-50">
                        ...
                        <label for="input-file-sql" class="cursor-pointer flex flex-col items-center gap-1.5">
                            ...
                            @if($fileSql)
                                <span class="text-xs font-semibold text-indigo-600">{{ $fileSql->getClientOriginalName() }}</span>
                                <span class="text-[11px] text-gray-400">Klik untuk ganti file</span>
                            @else
                                <span class="text-xs font-medium text-gray-700">Klik untuk memilih file .sql backup</span>
